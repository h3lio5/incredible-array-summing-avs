...
type AvsSubscriberer interface {
	SubscribeToNewTasks(newTaskCreatedChan chan *cstaskmanager.ContractIncredibleSummingTaskManagerNewTaskCreated) event.Subscription
	SubscribeToTaskResponses(taskResponseLogs chan *cstaskmanager.ContractIncredibleSummingTaskManagerTaskResponded) event.Subscription
	ParseTaskResponded(rawLog types.Log) (*cstaskmanager.ContractIncredibleSummingTaskManagerTaskResponded, error)
}
...
func BuildAvsSubscriberFromConfig(config *config.Config) (*AvsSubscriber, error) {
	return BuildAvsSubscriber(
		config.IncredibleSummingRegistryCoordinatorAddr,
...
func (s *AvsSubscriber) SubscribeToNewTasks(newTaskCreatedChan chan *cstaskmanager.ContractIncredibleSummingTaskManagerNewTaskCreated) event.Subscription {
...
func (s *AvsSubscriber) SubscribeToTaskResponses(taskResponseChan chan *cstaskmanager.ContractIncredibleSummingTaskManagerTaskResponded) event.Subscription {
...
func (s *AvsSubscriber) ParseTaskResponded(rawLog types.Log) (*cstaskmanager.ContractIncredibleSummingTaskManagerTaskResponded, error) {
	return s.AvsContractBindings.TaskManager.ContractIncredibleSummingTaskManagerFilterer.ParseTaskResponded(rawLog)
}
